...
class MentionText extends StatelessWidget {
  ...
  const MentionText({super.key, required this.text, this.style, this.mentionStyle, this.maxLines, this.overflow});
  ...
  Widget build(BuildContext context) {
    ...
    for (final match in mentionRegExp.allMatches(text)) {
      if (match.start > lastIndex) {
        children.add(TextSpan(text: text.substring(lastIndex, match.start), style: style));
      }
            ...
            ..onTap = () async {
              try {
                ...
                if (user != null && context.mounted) {
                  Navigator.of(context).push(MaterialPageRoute(builder: (context) => ProfileScreen(userId: user.uid)));
                } else {
                  Get.snackbar(
                    "User Not Found",
    ...
    return RichText(
      ...
      text: TextSpan(children: children, style: style ?? DefaultTextStyle.of(context).style),
    );
  }
}
